...
//getButton sirve para usar los botones de configuracion o logout
function getButton(){
    $admin = userType($_SESSION["id"]);
    ...
    }else if(isset($_REQUEST["administration"])){
        if($admin){
...
function userExists($email){
    $result = false;
    $conn = connectToDB();
    $user = consultDB("users",$email,"email");
    if($email == $user["email"]){
        $result = true;
    }
    return $result;
