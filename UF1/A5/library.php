...
//
function recoveryCookie($mail){
    $error = "";
    if(isset($_COOKIE["recoveryActive"])){
        $error = "Hay una recuperacion activa para el mail ".$_COOKIE["recoveryActive"];
    }  else{
        setcookie("recoveryActive",$mail, time() + (31*24*60*60));
        recoveryMail($mail);
...
function recoveryMail($mail){
    ...
    $mail->Body = '<html><body>Hola,</br> Usa el link de la linea inferior para cambiar su contraseña! </br>Link:<a href="https://dawjavi.insjoaquimmir.cat/jfuentes/UF1/A5/recoverPassword.php?">aquí</a> El link sera valido durante 24 horas</body></html>';
    $mail->CharSet = 'UTF-8';
    $mail->IsHTML(true);
    if ($mail->send()){
        header("location: login.php");
...
//
function recoveryDataTest($mail){
    $error = "";
    if(empty($mail)){
        $error = "*El campo \"Email\" es obligatorio.";
        ...
        if (!filter_var($mail, FILTER_VALIDATE_EMAIL)) {
            ...
            if($mail != $mailDB){
                $error = "Este mail no esta registrado.";

            } else {
                $error = recoveryCookie($mail);
